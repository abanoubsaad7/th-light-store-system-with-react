  ...
  const handleAddingFinalProduct = async (event) => {
    ...
    try {
      const res = await axios.post(
        "https://th-light-store-system.onrender.com/finalProducts/add-new-final-product",
        formData,
        config
      );
    ...
    <div className="pageContent">
      <form method="post" onSubmit={handleAddingFinalProduct}>
        <input
          type="text"
          name="name"
          placeholder="Name of the final product"
          style={{ marginRight: "2%" }}
          required
        />
        <input
          type="number"
          name="price"
          placeholder="price for one product"
          style={{ marginRight: "2%" }}
          required
        />
        <input
        ...
        <table className="table">
          ...
          <tbody id="item-table-body">
            {priceInputs.map((input, index) => (
              <tr key={index}>
                <td>
                  <MaterialsListBox nameOflistBox={`namesOfMaterialsUsed[${index}]`} />
                </td>
                <td>
                  <input
                    type="number"
                    className="price-input"
                    name={`numbersOfMatrialsUsed[${index}]`}
                    required
...
export default AddNewFinalProduct;
